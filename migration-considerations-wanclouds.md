...
The {{site.data.keyword.vpc-plus-migration}} tool duplicates your environment on {{site.data.keyword.cloud_notm}} Virtual Private Cloud and does not cancel your {{site.data.keyword.cloud_notm}} classic infrastructure. You can cancel your existing classic environment from the {{site.data.keyword.cloud_notm}} console.
...
You can migrate a VMDK-formatted image from classic to VPC by using the {{site.data.keyword.vpc-plus-migration}} tool. The {{site.data.keyword.vpc-plus-migration}} tool converts the VMDK image to qcow2, which is the supported format in VPC. The image must meet the image requirements that are listed in the [Considerations for virtual server instances](/docs/wanclouds-vpc-plus?topic=wanclouds-vpc-plus-migration-considerations#virtual-server-instances) section:
...
If you decide to use the {{site.data.keyword.vpc-plus-migration}} tool to convert and migrate your VMDK image, you must export the image to {{site.data.keyword.cos_full_notm}} before you begin using the tool. For more information, see [Upload data](/docs/cloud-object-storage?topic=cloud-object-storage-upload).
{: important}
...
If the {{site.data.keyword.vpc-plus-migration}} tool encounters a profile during discovery that doesn't match a supported profile, the closest matching profile is selected. You can change the selection when editing the discovered resources.
...
The {{site.data.keyword.vpc-plus-migration}} tool discovers virtual server instances that are associated with instance groups. You can migrate these instances with the {{site.data.keyword.vpc-plus-migration}} tool, but the tool does not set up auto scale policies in VPC. It's recommended that you migrate only one virtual server instance that is associated with an instance group and then set up auto scale policies with that virtual server instance. For more information, see [Creating an instance group for auto scaling](/docs/vpc?topic=vpc-creating-auto-scale-instance-group).

## Data storage considerations for attached, block, and file storage
{: #storage}

The {{site.data.keyword.vpc-plus-migration}} tool can discover the three types of storage used by virtual server instances in {{site.data.keyword.cloud_notm}} classic infrastructure:

1. Portable storage
2. Network file storage
3. Network-attached block storage (iSCSI) with file systems 

With {{site.data.keyword.vpc-plus-migration}} tool, you have the option to choose which data volumes to migrate. Only the volumes that you select will be migrated. 

If you have a virtual server instance that has already been migrated where you want to migrate secondary data volumes, use the _Content Data Migrator_ in the {{site.data.keyword.vpc-plus-migration}} tool.
...
2. You need to download the script that is provided by the {{site.data.keyword.vpc-plus-migration}} tool and run it on your classic virtual server instance with the required privileges. Instructions are provided in the script. The script installs the content migration agent that collects the necessary metadata that is needed for migration. The content migration agent collects information on attached storage, for example, name, partition information for block devices, mount points, file system types. The agent uploads the metadata to the {{site.data.keyword.vpc-plus-migration}} controller, providing an option later in the tool to select the wanted volumes for migration. For this to happen, your virtual server needs external connectivity to upload metadata files to the {{site.data.keyword.vpc-plus-migration}} controller.
...
The {{site.data.keyword.vpc-plus-migration}} tool can detect the SSH public keys that were added during the initial ordering process of your classic virtual server instances. If the SSH public keys aren't RSA or were added after you ordered your instances, then the {{site.data.keyword.vpc-plus-migration}} tool will require you to add a new SSH key.
...
The {{site.data.keyword.vpc-plus-migration}} tool does not convert network or host-based ACL rules. You will need to add these rules when editing your discovered resources or after you provision your VPC environment.
...
## Considerations for {{site.data.keyword.cloud_notm}} Kubernetes Service and {{site.data.keyword.redhat_openshift_notm}} migration
{: #kubernetes-migration}

With the {{site.data.keyword.vpc-plus-migration}} tool, you can discover and migrate your {{site.data.keyword.containerlong_notm}} or {{site.data.keyword.redhat_openshift_full}} cluster from classic to {{site.data.keyword.vpc_short}}. The {{site.data.keyword.vpc-plus-migration}} tool identifies all of your clusters and creates a workspace to help you manage your migration. You decide which cluster you want to migrate. 

You can migrate the following versions:
* {{site.data.keyword.cloud_notm}} Kubernetes Service version (latest supported 3)
* {{site.data.keyword.redhat_openshift_notm}} version (latest supported 3)
...
The open source tools Velero and Restic are installed in their unique namespace by {{site.data.keyword.vpc-plus-migration}} to back up and migrate your {{site.data.keyword.containerlong_notm}} or {{site.data.keyword.redhat_openshift_notm}} resources and components such as cluster-level resources, worker pools, PODs, namespaces, secrets, stateful sets, daemon sets, replica sets, jobs, cron jobs, and persistent volumes.
