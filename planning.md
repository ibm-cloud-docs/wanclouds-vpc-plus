...
--- | ---
1. Username | Create a username specifically for {{site.data.keyword.vpc-plus-migration}}. In the {{site.data.keyword.cloud_notm}} console, select **Manage > Access (IAM) > Users.** Select **Users** and then click **Invite user** the name of the user. This allows you to delete the username after your migration is complete. For more information, see [Inviting users to an account](/docs/account?topic=account-iamuserinv).
